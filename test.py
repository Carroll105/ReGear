...
def IFS_validation(X, Y, times,fold,select,tag):
    ...
    col = ['SVM', 'NBayes', 'KNN', 'DTree', 'GBDT', 'ELM',
           'LogisticRegression', 'RF', 'xgboost','BayesNetwork','lightGBM']
    score.columns = col
...
def TRank(data, label,times,fold,cnt,tag,geneOrResidue):
    ...
    result.to_csv(tag + '_Ttest_rank.txt', index=None, columns=col, sep="\t")
...
def WRank(data, label,times,fold,cnt,tag,geneOrResidue):
    ...
    print(result.head())
    col = [geneOrResidue, 'pvalue']
    result.to_csv(tag + '_Wtest_rank.txt', index=None, columns=col, sep="\t")
    print(select + ' selection finish...')
    print(select + " IFS validation start...")
...
def SVM_RFE(data, label,times,fold,cnt,tag,geneOrResidue):
    ...
    if geneOrResidue != "gene"and len(data)>20000:  # top 20000 pvalue
        order = pd.read_csv(tag + '_Ttest_rank.txt', index_col=0).iloc[list(range(20000))]
        # print(order)
...
def select_feature(code,filename,labelname,gene=True,times=20,fold=5,cnt=100):
    if gene:
        geneOrResidue="gene"
    else:
...
if __name__=='__main__':
    ...
    select_feature(code,filename, labelname,gene=False) # feature type is residue
